...
class RuntimeConfigTests(unittest.TestCase):
    ...
    def test_postgres_pilot_review_approve_materializes_canonical_rows(self) -> None:
        ...
        self.assertIn("item.value->>'retention_status'", sql_text)
        self.assertIn("document.retention_status", sql_text)
        self.assertIn("version.retention_status", sql_text)
        self.assertIn("THEN 'retired'", sql_text)
        self.assertIn("retired", sql_text)
        self.assertIn("evidence.payload->>'retention_status'", sql_text)
    ...
    def test_postgres_pilot_period_snapshot_reads_selected_approved_period(self) -> None:
        ...
        self.assertIn("LOWER(COALESCE(document.retention_status, 'active')) NOT IN ('scheduled_delete', 'deleted')", snapshot_query[0])
