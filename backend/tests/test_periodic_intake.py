...
class PeriodicIntakeTests(unittest.TestCase):
    ...
    def test_draft_validate_submit_approve_materializes_period_snapshot(self) -> None:
        ...
        self.assertEqual(snapshot["review_decision"]["review_task_id"], submitted["review_task"]["id"])
        self.assertEqual(snapshot["review_decision"]["assigned_to"], "reviewer-001")
        self.assertEqual(snapshot["review_decision"]["decided_by"], self.context.actor_id)
    ...
    def test_uploaded_clean_evidence_is_counted_and_sourced_in_approved_snapshot(self) -> None:
        ...
        self.assertNotIn("object_key", guarantee)
        self.assertEqual(guarantee["evidence_version_id"], upload["evidence_version_id"])
        self.assertEqual(guarantee["object_version"], "uploaded-demo")
        self.assertEqual(guarantee["document_hash"], "f" * 64)
        self.assertEqual(guarantee["content_type"], "application/pdf")
        self.assertEqual(guarantee["byte_size"], 4096)
