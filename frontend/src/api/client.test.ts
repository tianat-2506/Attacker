...
describe("api client trust headers", () => {
  ...
  it("maps periodic intake endpoints and preserves selected period in requests", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (url.includes("/periods/2026-07/snapshot")) return new Response(JSON.stringify({ data: { business_id: "BIZ-009", organization_id: "BIZ-009", period: submissionPayload.period, approved_version: 1, approved_at: "2026-07-01T00:10:00Z", review_decision: { review_task_id: "REV-1", assigned_to: "reviewer-001", assignment_reason: "auto_assigned_primary_org_reviewer", decided_by: "reviewer-001", decision: "approve", decision_note: "ok", decided_at: "2026-07-01T00:10:00Z" }, review_history: [{ review_task_id: "REV-1", submission_id: "SUB-1", review_status: "closed", assigned_to: "reviewer-001", decided_by: "reviewer-001", decision: "approve", decision_note: "ok", decided_at: "2026-07-01T00:10:00Z", submission_status: "approved", source: "manual", version: 1 }], sections: {}, financials: [], products: [], evidence: [], source_submission_ids: ["SUB-1"], advisory_notice: "Decision support only." } }), { status: 200 });
    ...
    expect(snapshot.reviewDecision?.reviewTaskId).toBe("REV-1");
    expect(snapshot.reviewDecision?.decisionNote).toBe("ok");
    expect(snapshot.reviewHistory?.[0].submissionId).toBe("SUB-1");
    expect(snapshot.reviewHistory?.[0].submissionStatus).toBe("approved");
  });
  ...
  it("maps trust foundation endpoints for consent evidence invoice and versioned runs", async () => {
    ...
    expect(ticket.classification).toBe("restricted_financial");
    ...
    expect(pendingTickets[0].classification).toBe("restricted_financial");
    ...
    const uploadCall = fetchMock.mock.calls.find(([url]) => String(url).endsWith("/evidence/upload-url")) as unknown as [string, RequestInit];
    expect(JSON.parse(String(uploadCall[1].body)).classification).toBe("restricted_financial");
